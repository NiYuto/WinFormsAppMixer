...
namespace WinFormsAppMixer
{
    public partial class Trim : Form
    {
        ...
        private void Trim_Load(object sender, EventArgs e)
        {
            ...
            {
                DialogResult result = MessageBox.Show("Для редактирования аудиофайла требуется формат .wav. Конвертировать файл?", "Внимание", MessageBoxButtons.OKCancel);
            ...
            {
                reader2 = new AudioFileReader(DataTransfer.InPath);
                customWaveViewer1.WaveStream = new WaveFileReader(reader2.FileName);
        ...
        public void TrimFile()
        {
            ...
            saveFileDialog.Filter = ".wav(*.wav)|*.wav";
            ...
            if (endPos <= startPos)
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                using (WaveFileWriter writer = new WaveFileWriter(saveFileDialog.FileName, reader2.WaveFormat))
                {
